...
int main()
{
	...
	while (!kbhit()) // 直到遇到按键退出
	{
		...
		/********************时间分段控制***************************/
		if ((tn = timeGetTime()) - t < 70000)
			choose(t1); // 随机烟花
		if ((tn = timeGetTime()) - t > 75000 && (tn = timeGetTime()) - t < 75010)
			style1();															  // 变换形式
		...
		if ((tn = timeGetTime()) - t > 105025 && (tn = timeGetTime()) - t < 175010)
			choose(t1); // 随机烟花
		if ((tn = timeGetTime()) - t > 180010 && (tn = timeGetTime()) - t < 180015)
			style1();																// 变换形式
...
{
	IMAGE fm;							   // 定义储存图片
	loadimage(&fm, "star.jpg", 1196, 797); // 将图片加载进去
...
void xinyue(DWORD &tx)
{
	...
	loadimage(&gm, "for you.png", 547, 167);
